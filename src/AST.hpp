...
struct StmtVisitor {
    ...
    virtual R visit(const std::shared_ptr<RangeForStmt>& stmt) = 0;
...
struct TypeCastExpr : Expr, public std::enable_shared_from_this<TypeCastExpr> {
    TypeCastExpr(std::shared_ptr<Type> type, std::shared_ptr<Expr> expression)
